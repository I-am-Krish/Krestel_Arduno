...
\begin{figure}[H]
    ...
    \draw[swire, green!55!black]
        (p2.east) to[out=0,in=180] (g16.west);
    \node[above, font=\tiny, text=green!40!black] at (5.75, 0.35)
        {\textbf{TX $\rightarrow$ RX2}\quad cross-connection};

    %% GPIO17 TX2 ──►  Pin3 RX    (orange, cross-connect)
    \draw[swire, orange!65!black]
        (g17.west) to[out=180,in=0] (p3.east);
    \node[above, font=\tiny, text=orange!55!black] at (5.75, 1.05)
        {\textbf{TX2 $\rightarrow$ RX}\quad cross-connection};

    %% Pin6 GND ──── ESP32 GND    (black, common ground)
    \draw[swire, black!55, <->]
        (p6.east) -- (gnd.west);
    \node[above, font=\tiny, text=black!60] at (5.75, -1.75)
        {Common Ground};
    ...
    \node[draw=codegray, dashed, rounded corners=2pt, fill=white,
        ...
        at (5.75, -0.5) {57{,}600 baud, 8N1, No parity};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        esl/.style={rectangle, draw=darkblue, thick,
                    ...
                    text width=5.8cm, rounded corners=2pt},
        %% PC GCS stage box
    ...
    \draw[->, skyblue, line width=2.8pt, >=stealth]
        (s6.east) -- (p1.west);
    \node[below, font=\footnotesize, text=skyblue, align=center] at (9.75, -0.4)
        {USB Micro-B cable $\cdot$ 115{,}200 baud $\cdot$ \texttt{/dev/ttyUSB0}};
    \node[below, font=\tiny, text=codegray, align=center] at (9.75, -0.72)
        {encrypted Kestrel binary packets};

    %% ==================================================================
    %% PC GCS COLUMN  (x = 19.5, top of column = USB RX at y = 0)
    %% ==================================================================
    \node[font=\normalsize\bfseries, text=successgreen] at (19.5, 1.0)
        {PC Ground Control Station};
    \node[font=\tiny, text=codegray] at (19.5, 0.6)
        {kestrel\_gcs\_usb.py (Python 3.10)};

    \node[usbl] (p1) at (19.5, 0)
        {\textbf{USB Serial Receive}\\[2pt]
         \texttt{pyserial} \texttt{serial.read(64)}\\[1pt]
         \texttt{/dev/ttyUSB0} @ 115{,}200 baud};

    \node[pcl] (p2) at (19.5, -1.6)
    ...
    \node[pcl, fill=yellow!8, draw=warnorange] (p5) at (19.5, -6.4)
    ...
    \node[anno] at (14.1, -0.8) {Received raw bytes\\(encrypted Kestrel)};
    \node[anno] at (14.1, -2.4) {Validated Kestrel\\frame buffer};
    \node[anno] at (14.1, -4.0) {Authenticated\\plaintext payload};
    \node[anno] at (14.1, -5.6) {Decoded message\\fields};
...
\begin{figure}[H]
    ...
    \node[bitbox, minimum width=2*\w cm] at (31*\w - 12.5*\w, 0) {prio};
    \node[bitbox, minimum width=2*\w cm] at (31*\w - 14.5*\w, 0) {strm[3:2]};
    \node[bitbox, minimum width=2*\w cm] at (31*\w - 16.5*\w, 0) {strm[1:0]};
    \node[bitbox, minimum width=6*\w cm] at (31*\w - 20.5*\w, 0) {plen[7:2]};
    \node[bitbox, minimum width=2*\w cm] at (31*\w - 24.5*\w, 0) {plen[1:0]};
    ...
    \node[bitbox, minimum width=2*\w cm] at (31*\w - 30.5*\w, 0) {seq[11:10]};
